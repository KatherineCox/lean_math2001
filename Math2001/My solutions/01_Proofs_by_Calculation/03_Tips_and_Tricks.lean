...
example {a b : ℚ} (h1 : a - 3 = 2 * b) : a ^ 2 - a + 3 = 4 * b ^ 2 + 10 * b + 9 :=
  calc
    a ^ 2 - a + 3
      = (a - 3 + 3)^2 - a + 3 := by ring
    _ = (2 * b + 3)^2 - a + 3 := by rw [h1]
    _ = 4 * b ^2 + 12 * b + 9 - a + 3 := by ring
    _ = 4 * b ^2 + 12 * b + 12 + (-1)*a := by ring
    _ = 4 * b ^2 + 12 * b + 12 + (-1)*(a -3 + 3) := by ring
    _ = 4 * b ^2 + 12 * b + 12 + (-1)*(2 * b + 3) := by rw [h1]
    _ = 4 * b ^2 + 12 * b + 12 - 2*b - 3 := by ring
    _ = 4 * b ^2 + 10 * b + 9 := by ring

-- Example 1.3.9 condensed
/- The above proof can be done in fewer steps, though it may be harder for the reader to follow: -/
example {a b : ℚ} (h1 : a - 3 = 2 * b) : a ^ 2 - a + 3 = 4 * b ^ 2 + 10 * b + 9 :=
  calc
    a ^ 2 - a + 3
...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1
      = (z^2)*(z^2) - (z^2)*z - z ^ 2 + 2* z + 1 := by ring
    _ = (z^2 -2 + 2)*(z^2 -2 +2) - (z^2 - 2 +2)*z - (z ^ 2 - 2 + 2)  + 2* z + 1 := by ring
    _ = (0+2)*(0+2) - (0+2)*z - (0 + 2) + 2* z + 1 := by rw [h1]
    _ = 2*2 - 2*z - 2 + 2*z + 1 := by ring
    _ = 3 := by ring

-- Example 1.3.10 condensed
/- The above proof can be done in fewer steps, though it may be harder for the reader to follow: -/
/- "Behind the scenes" this proof used polynomial division for the first step-/
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1
